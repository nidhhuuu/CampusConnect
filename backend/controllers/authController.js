...
export const authController = {
  ...
  login: (request, response) => { const missing = required(request.body, ['email', 'phone', 'password']); if (missing) return response.status(400).json({ message: `${missing} is required` }); const user = userModel.verify(request.body.email, request.body.phone, request.body.password); if (!user) return response.status(401).json({ message: 'Invalid email, phone number, or password' }); setSession(response, user); return response.json({ user }) },
